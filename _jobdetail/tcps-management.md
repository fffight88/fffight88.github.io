...
TCPS 관리자페이지의 `제품관리-템플릿수정`에서 나타나는 가격은 `product_detail_price` 테이블의 `templ_cost` 컬럼에 저장되어 사용되며, 수정하려고 하는 템플릿의 `product serial(p_serial)` 및 `pg_code`를 이용해 적절히 검색하여 요청받은 가격으로 업데이트 해준다.
