...
- [요청있을 때](#요청있을-때)
	...
	- [특정 제품만 안될 때](#특정-제품만-안될-때)
		- [배경이미지](#배경이미지)
		- [표시되어야할 단어나 문자열 등이 안나올 때 (구버전)](#표시되어야할-단어나-문자열-등이-안나올-때-구버전)
		- [현대일렉트릭 (크레디아) 주문수정요청 연동 안될 때](#현대일렉트릭-크레디아-주문수정요청-연동-안될-때)
	...
	- [명함 가변항목 텍스트 입력시 입력패턴 지정하기](#명함-가변항목-텍스트-입력시-입력패턴-지정하기)
	- [관리자페이지에서 생산팀이용내역 엑셀파일 다운로드 안될 때 조치](#관리자페이지에서-생산팀이용내역-엑셀파일-다운로드-안될-때-조치)
- [특정시점](#특정시점)
...
#### 표시되어야할 단어나 문자열 등이 안나올 때 (구버전)

개발자툴에서 해당 요소의 value값까지 잘 가져오는데 텍스트가 표현이 안 될 때에는 해당 값을 db에서 가져오는 function에서 `$_GLOBAL_CODES` 를 호출하지 않는지 확인한다.

호출한다면 `$_GLOBAL_CODES`에 해당 코드값이 등록이 되지 않은 경우이므로 `/home/podtest/inc/html_value.php` 를 열어 코드와 명칭을 등록한다.

<br>
<br>

#### 현대일렉트릭 (크레디아) 주문수정요청 연동 안될 때

https://btb.tarapod.co.kr:8443/v2/INTERLOCK/newprint_c.php 문서에서 작업

> [/assets/document/크레디아_연동%20인터페이스%20V3.3_210707_업데이트%20요청.pdf](/assets/document/크레디아_연동%20인터페이스%20V3.3_210707_업데이트%20요청.pdf) 참조
...
> 고객사정보 : mem_c, 부서정보 : mem_depth_name, 직책정보 : mem_l_info, tcps이용자계정 정보 : mem_table
...
1. 담당매니저가 요청할 때 지정할 부서그룹, 직책그룹 및 그 부서와 직책을 적용할 템플릿을 미리 만들고 요청해온다. 혹시 만들지 않았으면 해당 고객사페이지에 관리자계정으로 들어가서 `관리자페이지 - 기초자료관리` 화면에서 만든다.
	- 직책/직급 드롭박스만 추가하고 싶다면 `기초자료관리 - 이용자기초자료` 란에서 이미 추가할 수 있게 되어 있다. 여기에 추가한 직책/직급은 `mem_l_info` 테이블에 저장되며 따로 부서를 생성하지 않았다면 `mem_depth_serial` 컬럼에는 값이 0으로 들어가니 select로 정보를 가져올 때 해당 회사의 `c_serial`값과 `mem_depth_serial` = 0 을 조건으로 가져올 수 있다.
	- 만약 TCPS 관리자페이지의 `기업회원 상세보기` 화면에서 `조직도` 란에 계열사/부서/팀 등의 이름을 적으면 그 단계별로 해당 고객사페이지의 `기초자료관리` 화면에서 선택할 수 있고 새 항목을 등록할 수 있는데, 여기서 등록한 이름은 `mem_depth_name` 테이블에 저장되며 단계에 따라 `depth`값이 1, 2, 3이 부여된다.
...
1단계가 '구분', 2단계가 '부서'로 지정되어 있으며, 명함 템플릿번호에 따라 1단계 항목을 다르게 가진다. 1단계 항목에 따라 가지게 되는 2단계 '부서'의 리스트가 다르게 표현되며, 부서데이터뿐 아니라 직책데이터 또한 1단계항목에 따라 다르게 가져오도록 설계됨

#### 잡코리아(346)

와디즈와 거의 똑같이 되어있지만, 2단계 '부서'의 항목들이 `parent_id` 를 1단계항목의 시리얼번호로 갖지 않고 '0'을 가짐으로써 어떤 명함을 선택하더라도 부서데이터는 다 똑같이 보이도록 되어있다. 즉, 부서데이터를 가져오는 쿼리에서 와디즈는 where절에 `parent_id`를 1단계항목의 시리얼번호로 지정하지만 잡코리아는 1단계항목의 시리얼번호가 아닌 0을 지정함. 
...
`tpa001_model.php` 에서 피에스파인(380), 월드비전리전(360) 검색해서 코드 참조
...
### 명함 가변항목 텍스트 입력시 입력패턴 지정하기

1. `tpa001_model.php\get_input_form_etc` 에서 적용하려는 고객사 번호를 추가한 후 해당 고객사번호로 if절을 만들어 원하는 요소와 패턴을 입력. 등록된 패턴을 확인하거나 새로운 패턴(정규표현식)을 등록하려면 `application\views\view_com\tpa001\order_form_js.php` 에서 `patten_check` 로 검색하면 찾을 수 있다. 패턴 지정의 예를 들면,
```php
if ($this->session_tcps['c_serial'] == 391) // 391 에스더포뮬라
{
	$input_etc['hp'] = array('patten'=>"number_dot");
	$input_etc['tel'] = array('patten'=>"number_dot");
	$input_etc['fax'] = array('patten'=>"number_dot");
}
```
2. 기존 패턴이 아닌 새로운 패턴을 등록했다면 `application\views\view_com\tpa001\order_form_js.php\init_set_input_data` 에서 `keyup`으로 검색하여 keyup 이벤트 내에 다른 패턴이 명시되어 있는 곳에 새로 만든 패턴의 이름을 `or` 연산자로 함께 넣어준다.

<br>
<br> 

### 관리자페이지에서 생산팀이용내역 엑셀파일 다운로드 안될 때 조치

원인은 십중팔구 주문제목에 쿼리에 포함되면 안 되는 특수문자(&, = 등)이 포함되어 있기 때문.
1. 일단 에러내용을 확인하기 위해 `application\intra\order.php` 의 맨 상단에 다음 코드를 추가한다.
```php
error_reporting(E_ALL);
ini_set('display_errors', 1);
```
2. 개발자툴->네트워크->미리보기(preview)에 보면 에러가 엄청나게 많이 나와있는데 이 중 확인할 것은 xml파일 로드시 오류가 발생했다면서 파일의 경로명을 보여주는 에러이다. 해당 파일을 열어보면 보통 주문제목에 특수문자가 입력되어있는데 이 특수문자를 허용가능한 특수문자로 바꾸거나 없애거나 인코딩하거나... 등의 조치를 취한다.
3. xml파일 수정으로도 안 되면 `t_order`, `p_order` 테이블에서 해당 주문을 찾아 마찬가지로 특수문자에 조치를 취해 준다.
...
* 주문 관련 파일 저장위치 : `/data/ICM_DATA/templ_storage/tpa001/(수주날짜폴더)/(고객사코드_t_serial폴더)/`
* 명함템플릿 파일 저장위치 : `/home/podtest/tpa001_templ/bg/(템플릿시리얼폴더)`
...
* 즉, TCPS 화면을 사용하지 않더라도 고객사정보, 단가정보, 제품정보를 다 등록하고 데이터를 가져오면서 데이터 내용에 따라 템플릿번호도 함께 p_order 테이블에 삽입이 되어야 함.(예: 페타애드텍)
