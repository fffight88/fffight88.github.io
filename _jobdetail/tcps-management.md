...
- [요청있을 때](#요청있을-때)
	- [생산팀 단가 신규 생성 및 적용](#생산팀-단가-신규-생성-및-적용)
	...
	- [생산팀에서 명함주문 이미지파일 문제있다고 문의들어올 때](#생산팀에서-명함주문-이미지파일-문제있다고-문의들어올-때)
- [특정시점](#특정시점)
...
### 생산팀에서 명함주문 이미지파일 문제있다고 문의들어올 때

> 원래는 이 부분이 없어야하는데 보여요.
> 원래는 이 부분이 보여야하는데 안보여요.

문제없이 잘 되다가 한 번씩 이런 문의가 들어오는 경우라면 소스코드상 문제라기보단 실행 중 오류발생으로 xml파일 생성이 잘못되었기 때문일 가능성이 매우매우 크다.

1. FTP로 서버 접속해서 `home\podtest\templ_storage` 디렉토리에서 해당 주문의 파일이 들어있는 디렉토리를 찾아 들어간다. `p_order` 테이블에서 해당 주문을 검색해보면 `folder` 컬럼에서 폴더명을 확인할 수 있다.
2. 앞면 xml파일과 뒷면 xml파일이 따로 있을터인데 공통적으로 다운로드시 생산팀에서 원래의 이미지와 다르게 보이게 되어있는 템플릿이라면 <bg> 태그에 `print_cmyk_name` 속성이 지정되어 있고 올바른 pdf파일명이 기재되어 있어야 한다. 이 속성이 존재하지 않는다면
3. `print_cmyk_name="print_f_6457.pdf"` 이런식으로 속성을 추가해주는데 앞면만 다르다면 앞면에만, 뒷면만 다르다면 뒷면에만, 양면 모두 다르다면 양면에 추가해준다. 6457 자리에 해당템플릿의 시리얼번호를 기재한다.
